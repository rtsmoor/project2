...
void Detectie() {
  ...
  // Calculating the distance
  distanceLeft = ((durationL * 0.034) / 2);
  distanceRight = ((durationR * 0.034) / 2);
  ...
  // Prints the distance on the Serial Monitor
  Serial.print("Distance on left side: ");
  Serial.print(distanceLeft);
  Serial.println(" cm");
  Serial.print("Distance on right side: ");
 ...
 if(distanceLeft == distanceRight){
  if(distanceRight <=4 || distanceLeft <= 4){
    robotReverse();
    robotLeft();
  }
